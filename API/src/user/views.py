...
from src import BaseView, limiter, db, sms
from src import api
from src.user.schemas import UserSchema
from src.utils.api import set_user
from src.utils.methods import List, Fetch, Create, Update
from .models import User
from .resources import UserResource, DeviceResource, RiderResource
...
class UserVerifyResource(Resource):
    ...
    def post(self):
        data = request.json
        if True: #redis_store.get('user:' + data['mobile_number']) and redis_store.get(data['mobile_number']).decode('utf-8') == str(data['otp']):
            #ser, errors = self.schema().load(json.loads(redis_store.get('user:' + data['mobile_number']).decode('utf-8')))
            user, errors = self.schema().load(json.loads(self.model.query.filter(self.model.mobile_number==data['mobile_number']).first()))
            if errors:
                return make_response(jsonify(errors), 400)
            try:
                user.active = True
...
class CustomerVerifyResource(Resource):
    ...
    def post(self):
        data = request.json
        user = self.model.query.filter(self.model.mobile_number == data['mobile_number']).first()
        if user: #redis_store.get(data['mobile_number']).decode('utf-8') == data['otp']:
